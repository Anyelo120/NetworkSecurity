...
public class RedisStorageProvider implements StorageProvider {
    ...
    public void saveIP(String ip, boolean isBlocked, String country, String continent) {
        try (Jedis jedis = jedisPool.getResource()) {
            ...
            if (isBlocked) {
                jedis.sadd("ip_index:blocked", ip);
                jedis.srem("ip_index:allowed", ip);
            } else {
                jedis.sadd("ip_index:allowed", ip);
                jedis.srem("ip_index:blocked", ip);
            }
        } catch (Exception e) {
    ...
    public int countBlockedIPs() {
        try (Jedis jedis = jedisPool.getResource()) {
            return Math.toIntExact(jedis.scard("ip_index:blocked"));
        } catch (Exception e) {
    ...
    public int countAllowedIPs() {
        try (Jedis jedis = jedisPool.getResource()) {
            return Math.toIntExact(jedis.scard("ip_index:allowed"));
        } catch (Exception e) {
